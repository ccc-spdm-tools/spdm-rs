...
pub struct VendorDefinedStructEx {
    pub vendor_defined_request_handler_ex:
        for<'a> fn(&mut ResponderContext, &[u8], &'a mut Writer) -> (SpdmResult, Option<&'a [u8]>),
    pub vdm_handle: usize, // interpreted/managed by User
}
...
static VENDOR_DEFNIED_DEFAULT_EX: VendorDefinedStructEx = VendorDefinedStructEx {
    vendor_defined_request_handler_ex: |_responder_context: &mut ResponderContext,
                                        _bytes: &[u8],
                                        _writer: &mut Writer|
...
pub fn vendor_defined_request_handler_ex<'a>(
    ...
    if let Ok(vds) = VENDOR_DEFNIED_EX.try_get_or_init(|| VENDOR_DEFNIED_DEFAULT_EX) {
        (vds.vendor_defined_request_handler_ex)(responder_context, bytes, writer)
    } else {
