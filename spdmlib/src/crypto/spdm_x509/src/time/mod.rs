...
impl Time {
    ...
    ///
    /// Returns `Err` if the system clock is unavailable (e.g. `no_std`
    /// without a clock source).
    #[cfg(feature = "std")]
    pub fn now() -> core::result::Result<Self, &'static str> {
        use std::time::SystemTime;
        let duration = SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .map_err(|_| "system clock before UNIX epoch")?;
        let secs = duration.as_secs();
        // Convert to der::DateTime
        let dt = der::DateTime::from_unix_duration(core::time::Duration::from_secs(secs))
            .map_err(|_| "failed to convert timestamp")?;
        let gen_time = GeneralizedTime::from_date_time(dt);
        Ok(Time::GeneralizedTime(gen_time))
    }

    /// Get the current time (no_std fallback — always fails).
    #[cfg(not(feature = "std"))]
    pub fn now() -> core::result::Result<Self, &'static str> {
        Err("system clock unavailable (no_std)")
    }
