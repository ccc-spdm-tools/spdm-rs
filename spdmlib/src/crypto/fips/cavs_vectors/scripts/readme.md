...
PQC vectors use [`pqc_cavs_to_rust.py`](pqc_cavs_to_rust.py). Official public
NIST ACVP sample projections are downloaded at a pinned ACVP-Server commit and
verified by [`pull_nist_pqc_vectors.py`](pull_nist_pqc_vectors.py). These are
NIST-hosted sample artifacts, not results from a credentialed ACVTS validation
session. The generator validates algorithms, modes, parameter sets, test IDs,
and field sizes, and stores SHA-384 fingerprints where full outputs are not
needed at runtime.
...
```
cd <WORKDIR>/spdm-rs/spdmlib/src/crypto/fips/cavs_vectors
python3 ./scripts/pqc_cavs_to_rust.py --algorithm ml-dsa \
	-i ../../../../../external/aws-lc-rs/aws-lc-rs/tests/data/MLDSA_87_ACVP_keyGen.txt \
	-o ml_dsa_87_key_gen.rs
```

ML-DSA-87 deterministic signature generation and signature verification
use the external, pure interface used by SPDM. Signature generation uses NIST
`tgId=5`, `tcId=71`; expected-failure verification uses NIST `tgId=5`,
`tcId=66`.
```
cd <WORKDIR>/spdm-rs/spdmlib/src/crypto/fips/cavs_vectors
tmp_dir=$(mktemp -d)
python3 ./scripts/pull_nist_pqc_vectors.py "$tmp_dir"
python3 ./scripts/pqc_cavs_to_rust.py --algorithm ml-dsa-signature \
	-i "$tmp_dir/ml-dsa-siggen.json" \
	--verify-input "$tmp_dir/ml-dsa-sigver.json" \
	--group-id 5 --test-id 71 \
	...
	-o ml_dsa_87_sig_gen_ver.rs
rm -rf "$tmp_dir"
```

ML-KEM-1024 key generation and encapsulation/decapsulation
```
cd <WORKDIR>/spdm-rs/spdmlib/src/crypto/fips/cavs_vectors
tmp_dir=$(mktemp -d)
python3 ./scripts/pull_nist_pqc_vectors.py "$tmp_dir"
python3 ./scripts/pqc_cavs_to_rust.py --algorithm ml-kem \
	-i "$tmp_dir/ml-kem-keygen.json" \
	--verify-input "$tmp_dir/ml-kem-encap-decap.json" \
