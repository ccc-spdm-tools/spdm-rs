...
impl ResponderContext {
    ...
    pub fn verify_spdm_certificate_chain(&mut self) -> SpdmResult {
        ...
        if !cert_chain_provisioned {
            // No trust anchor provisioned for the peer (requester) cert chain.
            // This is acceptable when mutual auth is not mandatory — the cert
            // chain integrity was still verified via signature checks above.
            info!("No trust anchor provisioned - skipping trust anchor verification\n");
        }
