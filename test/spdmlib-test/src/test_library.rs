...
fn test_case3_spdm_context_export_import_boundary_conditions() {
    ...
    let max_cert_data = spdmlib::protocol::SpdmCertChainData {
        data_size: spdmlib::config::MAX_SPDM_CERT_CHAIN_DATA_SIZE as u32,
        data: vec![0xCD; spdmlib::config::MAX_SPDM_CERT_CHAIN_DATA_SIZE]
            .into_boxed_slice()
            .try_into()
            .unwrap(),
    };
    for root_cert in &mut original_context.provision_info.peer_root_cert_data {
        *root_cert = Some(max_cert_data.clone());
    }
    original_context.provision_info.my_pub_key = Some(max_cert_data.clone());
