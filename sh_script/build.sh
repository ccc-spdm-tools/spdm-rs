...
run_basic_test() {
    ...
    echo_command cargo test -p spdmlib --no-default-features \
        --features="std,spdm-ring,fips,sha256,sha384,sha512,aes-256-gcm,rsa-pkcs1,ecdsa-p256,ecdsa-p384,ecdh-p256,ecdh-p384" \
    ...
    echo_command cargo test -p spdmlib_crypto_aws_lc --no-default-features \
        --features="full,fips" test_spdm_fips_self_tests -- --test-threads=1
